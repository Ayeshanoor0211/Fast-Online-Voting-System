...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (userNameElement) {
        userNameElement.textContent = user.Name || 'Faculty';
    }
...
async function loadActiveElections() {
    ...
    try {
        ...
        // Filter elections relevant to faculty
        const facultyElections = data.elections ? data.elections.filter(e => e.PositionName.toLowerCase().includes('faculty')) : [];

        if (countElement) countElement.textContent = facultyElections.length;
        listContainer.innerHTML = ''; // Clear spinner

        if (facultyElections.length === 0) {
            listContainer.innerHTML = `
                <div class="col-12 text-center py-5">
                    <i class="fas fa-box-open fa-3x text-muted mb-3"></i>
                    <h5 class="text-muted">No active elections for faculty at the moment.</h5>
                </div>`;
            return;
        }

        for (const election of facultyElections) {
...
async function createElectionCard(election) {
    ...
    return `
        <div class="col-lg-6 mb-4">
            <div class="card h-100 shadow-sm border-0 rounded-3" style="border-left: 4px solid #43a047 !important;">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <span class="badge bg-success">FACULTY ELECTION</span>
                        <small class="text-muted">Ends in ${endsInDays} days</small>
                    </div>
                    <h4 class="card-title fw-bold text-success">${election.Title}</h4>
